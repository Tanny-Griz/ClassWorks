...
function createPizza(name, arrOfIds) {
  ...
  for (let value of arrOfIds) {
    ...
    for (let comp of compositionList){
      if (value == comp.id) {
        pizzaObj.composition.push(comp.name);
        pizzaObj.caloricity += comp.caloricity;
        pizzaObj.price += comp.price;
      }
    }
  }
  pizzaList.push(pizzaObj); // добавили наш обьект в общий массив
