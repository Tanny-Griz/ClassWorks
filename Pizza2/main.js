...
function getPizzaComposition(comp) {
  let compositionName = comp.toLowerCase();
  let sortedArr = [];
  for (let i = 0; i < pizzaList.length; i++) {
    for (let j = 0; j < pizzaList[i].composition.length; j++) {
      if (compositionName == pizzaList[i].composition[j]) {
        sortedArr.push(pizzaList[i]);
      }
    }
  }
  return sortedArr;
}
console.log(getPizzaComposition('Моцарелла'));

function createPizza(name, arrOfIds) {
  let pizzaObj = {};
  pizzaObj.id = pizzaList[pizzaList.length - 1].id + 1; // посл. индекс
  pizzaObj.caloricity = 1000;
  pizzaObj.composition = [];
  pizzaObj.price = 100;
  
  for (let value of arrOfIds) {
    for (let composition of compositionList) {
      if (value == compositionList.id) {
        pizzaObj.composition.push(composition.name);
        pizzaObj.caloricity += composition.caloricity;
        pizzaObj.price += composition.price;
      }
    }
  }
